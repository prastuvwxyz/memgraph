...
func TestSkipUnchangedChecksum(t *testing.T) {
	...
	f := makeFile("/notes/foo.md", "abc123")
	if _, err := db.IndexFile(f); err != nil {
...
func TestWalkTempDir(t *testing.T) {
	...
	updated, total, err := Walk(db, dir, nil, false)
	if err != nil {
		t.Fatalf("Walk: %v", err)
	}
	if total != 3 {
	...
	updated2, total2, err := Walk(db, dir, nil, false)
	if err != nil {
...
func TestWalkExclude(t *testing.T) {
	...
	updated, total, err := Walk(db, dir, []string{"drafts"}, false)
	if err != nil {
